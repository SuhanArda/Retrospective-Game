...
export class SignalRGameTransport implements GameTransport {
  ...
  useAbility(command: UseAbilityCommand) {
    if (command.abilityId === 'rocket' || this.currentRoundId === 0) return;
  ...
  private bindRoomEvents() {
    ...
    this.roomDisposers.push(
      ...
      this.roomClient.on('retroRushSnapshot', (snapshot) => { this.currentRoundId = snapshot.roundId; this.emit({ type: 'retroSnapshot', snapshot }); }),
      ...
      this.roomClient.on('retroRushRoundStarted', (snapshot) => { this.currentRoundId = snapshot.roundId; this.emit({ type: 'retroRoundStarted', snapshot }); }),

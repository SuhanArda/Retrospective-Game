...
describe('runtime configuration', () => {
  ...
  it('accepts explicitly configured SignalR and platform boundaries', () => expect(parseRuntimeConfig({ VITE_TRANSPORT_MODE: 'signalr', VITE_HUB_URL: 'https://example.test/hub', VITE_PLATFORM_URL: '/platform/', VITE_API_URL: 'https://rooms.example.test', VITE_AI_BOT_URL: 'https://questions.example.test' })).toEqual({ transportMode: 'signalr', hubUrl: 'https://example.test/hub', platformUrl: '/platform/', roomApiUrl: 'https://rooms.example.test', aiBotUrl: 'https://questions.example.test' }));
  it('keeps AI questions optional', () => expect(parseRuntimeConfig({ VITE_PLATFORM_URL: '/', VITE_API_URL: 'https://rooms.example.test' }).aiBotUrl).toBeNull());
  it('falls back safely for unknown modes', () => expect(parseRuntimeConfig({ VITE_TRANSPORT_MODE: 'other', VITE_PLATFORM_URL: '/', VITE_API_URL: 'https://rooms.example.test', VITE_AI_BOT_URL: 'https://questions.example.test' })).toEqual({ transportMode: 'mock', platformUrl: '/', roomApiUrl: 'https://rooms.example.test', aiBotUrl: 'https://questions.example.test' }));

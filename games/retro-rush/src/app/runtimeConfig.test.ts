...
describe('runtime configuration', () => {
  it('defaults to mock mode and local platform services', () => expect(parseRuntimeConfig({})).toEqual({ transportMode: 'mock', platformUrl: 'http://localhost:5173', roomApiUrl: 'http://localhost:5281' }));
  ...
  it('falls back safely for unknown modes', () => expect(parseRuntimeConfig({ VITE_TRANSPORT_MODE: 'other' })).toEqual({ transportMode: 'mock', platformUrl: 'http://localhost:5173', roomApiUrl: 'http://localhost:5281' }));

...
export const RUS_RULETI_GAME_ID = 'rus-ruleti';

export interface RusRuletiRuntimeConfig {
  platformUrl: string;
  apiUrl: string;
}

export function parseRusRuletiRuntimeConfig(
  env: Record<string, string | boolean | undefined>,
): RusRuletiRuntimeConfig {
  return {
    platformUrl:
...
export const rusRuletiRuntimeConfig = parseRusRuletiRuntimeConfig(import.meta.env);

export function resolveRusRuletiLaunchContext(
  search: string,
  storage: Storage,
  ...
  return context?.gameId === RUS_RULETI_GAME_ID ? context : null;
}

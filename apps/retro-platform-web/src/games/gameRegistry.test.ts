...
describe('game registry', () => {
  it('registers Retro Rush, Spin the Bottle, Rus Ruleti, and Draw & Guess as uniquely identified playable games', () => {
    const available = gameRegistry.filter((game) => game.status === 'available');
    expect(available.map((game) => game.id)).toEqual(['retro-rush', 'spin-the-bottle', 'rus-ruleti', 'draw-and-guess']);
